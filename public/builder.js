...
// Trigger → Init → 🧠 Context → [ StepAgent → JSON → Run → Record ] × N → Summary
...
(function () {
  ...
  function baseWorkflow(name){
    return { name, nodes:[], connections:{}, active:false,
      settings:{ executionOrder:"v1" }, staticData:{}, __occ:new Set() };
  }
  ...
  function blockCells(x,y,w=FOOT.w,h=FOOT.h){
    const sx=snapX(x), sy=snapY(y), out=[];
    for(let dx=0;dx<w;dx++) for(let dy=0;dy<h;dy++) out.push(`${sx+dx*GRID.cellW}:${sy+dy*GRID.cellH}`);
    return out;
  }
  function reserve(wf,x,y,w=FOOT.w,h=FOOT.h){ blockCells(x,y,w,h).forEach(c=>wf.__occ.add(c)); return [snapX(x), snapY(y)]; }
  function freeY(wf,x,y,w=FOOT.w,h=FOOT.h){
    let cy=snapY(y); while(blockCells(x,cy,w,h).some(c=>wf.__occ.has(c))) cy+=GRID.cellH; return cy;
  }
  function addNode(wf,node){
    node.name=uniqueName(wf,node.name);
    if(Array.isArray(node.position)){
      const x=snapX(node.position[0]), y=freeY(wf,x,node.position[1]);
      node.position=[x,y]; reserve(wf,x,y);
    }
  ...
  // NOTE: these now take `wf` explicitly
  const modelLm  = (wf, role, x, y)=> addNode(wf,{ id:uid('lm'), name:`${role} · OpenAI Chat Model`,
    type:"@n8n/n8n-nodes-langchain.lmChatOpenAi", typeVersion:1.2, position:pos(x,y),
  ...
  const parser   = (wf, role, x, y, schema)=> addNode(wf,{ id:uid('op'), name:`${role} · Structured Parser`,
    type:"@n8n/n8n-nodes-langchain.outputParserStructured", typeVersion:1.3, position:pos(x,y),
...
- 'title' must be short & scenario-specific.
- Use channel_bundle for messages.
- Choose 'END' if nothing left.
JSON only.`;
...
return exec().then(r => [{ ...$json, __exec:r }]);`
  ...
  function addStepAgent(wf, x, y, stepName){
    ...
    const lm   = modelLm(wf, role, x, y+152);
    const prs  = parser(wf, role, x+144, y+260, SCHEMA_STEP);
  ...
  function buildWorkflowJSON(scenario, industry, opts={}){
    ...
    const trig = manual(wf, L.start.x, L.start.y);
    ...
    // 🧠 Context agent
    const ctxLm  = modelLm(wf,'🧠 Context', L.start.x + 2*L.stepX, L.start.y+152);
    const ctxPr  = parser(wf,'🧠 Context',  L.start.x + 2*L.stepX + 144, L.start.y+260, SCHEMA_CONTEXT);
    const ctxAg  = agentNode(wf,'🧠 Context', L.start.x + 2*L.stepX, L.start.y, sysContext, userContext(scenario, industry));
    ...
    wf.staticData.__design = {
      ...
      notes:'Each step = Agent → JSON → Run → Record. Customize prompts in userStep(); add/remove steps via opts.steps.'
    };
    return wf;
  }
